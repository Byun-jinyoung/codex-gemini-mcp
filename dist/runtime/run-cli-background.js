...
async function persistStatus(statusFile, status) {
    try {
...
export async function runCliBackground(request) {
    ...
    const backgroundLogContext = logContext
        ? { ...logContext, jobId }
        : undefined;
    ...
    const complete = async (nextStatus, error, errorCode) => {
        if (terminal) {
            return;
        }
        terminal = true;
    ...
    child.on("close", (code) => {
        ...
        void complete("failed", `${command} exited with code ${code}: ${stderr.trim() || "no stderr"}`, "CLI_NON_ZERO_EXIT");
    });
    child.unref();
    return {
        provider,
