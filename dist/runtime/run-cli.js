...
export function runCli(command, args, timeoutMs = getDefaultTimeoutMs(), cwd, logContext) {
    return new Promise((resolve, reject) => {
        ...
        const rejectWithLog = (errorCode, message, stderrText) => {
            if (settled) {
                return;
            }
            settled = true;
        ...
        child.on("close", (code) => {
            ...
            rejectWithLog("CLI_NON_ZERO_EXIT", `${command} exited with code ${code}: ${stderr.trim() || "no stderr"}`);
        });
    });
}
