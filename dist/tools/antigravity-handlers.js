...
function getAgyLogPath(cwd, requestId) {
    return path.join(getRuntimeDir(cwd), "agy-logs", `${requestId}.log`);
}
async function ensureLogDir(logFile) {
    await mkdir(path.dirname(logFile), { recursive: true });
}
async function parseSessionFromLogFile(logFile, stdout, existingSessionId) {
...
export async function handleAskAntigravity(input) {
    ...
    if (input.background ?? true) {
        const metadata = await runCliBackground({
            ...
            cwd: input.working_directory,
            logContext,
            transformResponse: (stdout) => parseSessionFromLogFile(logFile, stdout, input.session_id),
        });
        return JSON.stringify(metadata);
    }
    const stdout = await runCli(command.command, command.args, timeoutMs, input.working_directory, logContext);
